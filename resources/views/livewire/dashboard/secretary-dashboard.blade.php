                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors {{ $appointment->status === 'seen' ? 'opacity-60' : '' }}">
                            ...
                            <td class="px-6 py-4 flex items-center gap-3">
                                ...
                                <div class="flex items-center gap-1 bg-gray-50 p-1 rounded-lg">
                                    <button wire:click="editPatient({{ $appointment->patient_id }})" class="p-1.5 text-gray-400 hover:text-purple-600 hover:bg-white rounded-md transition-all" title="{{ __('Edit Patient Data') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <button wire:click="editAppointment({{ $appointment->id }})" class="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-white rounded-md transition-all" title="{{ __('Edit Appointment Time') }}">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    </button>
                                </div>

                                <button wire:click="deleteAppointment({{ $appointment->id }})" wire:confirm="{{ __('Are you sure you want to delete this appointment?') }}" class="p-1.5 text-gray-400 hover:text-red-600 transition-colors" title="{{ __('Delete') }}">
