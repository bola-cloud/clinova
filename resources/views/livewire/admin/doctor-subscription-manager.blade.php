<div class="p-6 space-y-8 animate-fade-in text-right" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-3xl font-black text-slate-900 tracking-tight">{{ __('Manage Subscription') }}</h1>
                <p class="text-slate-500 font-bold flex items-center gap-2">
                    <span class="text-indigo-600">{{ $doctor->name }}</span>
                    <span class="w-1 h-1 bg-slate-300 rounded-full"></span>
                    <span>{{ $doctor->email }}</span>
                </p>
            </div>
        </div>
        <a href="{{ route('admin.dashboard') }}" wire:navigate class="px-6 py-3 bg-white text-slate-900 border border-slate-200 rounded-2xl font-black text-sm flex items-center gap-2 hover:bg-slate-50 transition-all shadow-sm">
        ...
        <div class="lg:col-span-1 space-y-8">
            ...
            <div class="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                ...
                <div class="relative z-10 space-y-6">
                    ...
                    <div>
                        <h4 class="text-slate-400 text-xs font-black uppercase tracking-widest mb-1">{{ __('Subscription Plan') }}</h4>
                        <p class="text-3xl font-black capitalize tracking-tight">{{ __($doctor->subscription_plan ?: 'None') }}</p>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <h4 class="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">{{ __('Expiry Date') }}</h4>
                            <p class="text-sm font-black">{{ $doctor->subscription_expires_at?->format('Y-m-d') ?: 'N/A' }}</p>
                        </div>
                        <div>
                            <h4 class="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">{{ __('Payment Status') }}</h4>
                            <p class="text-sm font-black {{ $doctor->is_paid ? 'text-emerald-400' : 'text-amber-400' }}">
                                {{ $doctor->is_paid ? __('Collected') : __('Not Collected') }}
                            </p>
                        </div>
                    </div>
                    ...
                    @if($doctor->subscription_expires_at)
                        ...
                        <div class="pt-4 border-t border-white/10">
                            <div class="flex justify-between text-[10px] font-black uppercase mb-2">
                                ...
                                <span class="{{ $daysLeft > 5 ? 'text-emerald-400' : 'text-rose-400' }}">{{ max(0, $daysLeft) }} {{ __('Days') }}</span>
                            </div>
                            <div class="h-2 bg-white/5 rounded-full overflow-hidden">
                                <div class="h-full {{ $daysLeft > 5 ? 'bg-emerald-500' : 'bg-rose-500' }} rounded-full transition-all duration-1000" style="width: {{ min(100, max(0, ($daysLeft / 30) * 100)) }}%"></div>
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-100/50">
                ...
                <form wire:submit="saveSubscription" class="space-y-5">
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest mr-1">{{ __('Plan') }}</label>
                        ...
                        @error('plan') <span class="text-rose-500 text-[10px] font-black">{{ $message }}</span> @enderror
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest mr-1">{{ __('Start Date') }}</label>
                        ...
                        @error('start_date') <span class="text-rose-500 text-[10px] font-black">{{ $message }}</span> @enderror
                    </div>

                    @if($plan === 'trial')
                    <div class="space-y-2 animate-slide-down">
                        <label class="text-[10px] font-black text-amber-600 uppercase tracking-widest mr-1">{{ __('Trial Duration (Days)') }}</label>
                        ...
                        @error('trial_days') <span class="text-rose-500 text-[10px] font-black">{{ $message }}</span> @enderror
                    </div>
                    @endif

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest mr-1">{{ __('Payment Price') }} (EGP)</label>
                        ...
                        @error('price') <span class="text-rose-500 text-[10px] font-black">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 group transition-all hover:bg-emerald-50/50 hover:border-emerald-100">
                        ...
                        <span class="text-sm font-black text-slate-900 group-hover:text-emerald-700 transition-all">{{ __('Payment Collected') }}</span>
                    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden h-full">
                <div class="p-8 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
                    <div>
                        <h2 class="text-xl font-black text-slate-900 tracking-tight">{{ __('Subscription History') }}</h2>
                        <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">{{ __('Past payments & upcoming slots') }}</p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-right">
                        <thead class="bg-slate-50/50 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
                            <tr>
                                ...
                                <th class="px-8 py-5 text-center">{{ __('Amount') }}</th>
                                <th class="px-8 py-5 text-center">{{ __('Status') }}</th>
                                <th class="px-8 py-5 text-center">{{ __('Actions') }}</th>
                            </tr>
                        </thead>
                            ...
                            <tr class="hover:bg-indigo-50/20 transition-all group">
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl flex items-center justify-center
                                            {{ $record->plan_name === 'trial' ? 'bg-amber-100 text-amber-600' : '' }}
                                            {{ $record->plan_name === 'monthly' ? 'bg-indigo-100 text-indigo-600' : '' }}
                                            {{ $record->plan_name === 'yearly' ? 'bg-purple-100 text-purple-600' : '' }}
                                        ">
                                            @if($record->plan_name === 'trial')
                                        ...
                                        <span class="font-black text-slate-900 capitalize">{{ __($record->plan_name) }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">
                                            {{ $record->start_date->diffInDays($record->end_date) }} {{ __('Days') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-8 py-6 text-center font-black text-slate-900 tracking-tighter">
                                    {{ number_format($record->amount, 2) }} <span class="text-[10px] text-slate-400">EGP</span>
                                </td>
                                <td class="px-8 py-6 text-center">
                                    <button wire:click="togglePaidStatus({{ $record->id }})" class="group/paid relative inline-flex">
                                        @if($record->is_paid)
                                ...
                                <td class="px-8 py-6 text-center">
                                    <button onclick="confirm('{{ __('Are you sure?') }}') || event.stopImmediatePropagation()" 
                                            wire:click="deleteSubscription({{ $record->id }})" 
                            ...
                            <tr>
                                <td colspan="5" class="px-8 py-32 text-center">
                                    <div class="flex flex-col items-center gap-4 text-slate-300">
                                        ...
                                        <p class="font-black italic text-lg uppercase tracking-widest">{{ __('No history recorded for this doctor.') }}</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
